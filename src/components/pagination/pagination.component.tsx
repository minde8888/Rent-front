...
export default function Pagination({ firstPage, lastPage, nextPage, previousPage, pageNumber, pageSize, totalPages, totalRecords }: Props) {
    ...
    return (
        <nav className="pagination" aria-label="Pagination">
            <button onClick={() => { onClick(previousPage) }} disabled={navPageFirst === null}>
                Previous
            </button>
            {pageNum.map((pageNum, i) => (
                <button key={i} onClick={() => { onClick(pageNum) }} disabled={pageNum === pageNumber}>
                    {pageNum}
                </button>
            ))}
            <button onClick={() => { onClick(nextPage) }} disabled={navPageThird === null}>
                Next
            </button>
        </nav>
    );
